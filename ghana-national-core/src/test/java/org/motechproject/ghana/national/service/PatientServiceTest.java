...
public class PatientServiceTest {
    ...
    @Test
    public void shouldNotReturnAPatientInSearchIfMarkedAsDead() {
        Patient alivePatient = new Patient(new MRSPatient(null, new MRSPerson(), null));
        Patient deadPatient = new Patient(new MRSPatient(null, new MRSPerson().dead(true), null));

        when(mockAllPatients.search(anyString(), anyString())).thenReturn(Arrays.asList(alivePatient, deadPatient));
        assertThat(patientService.search("some name", "some motech id"), is(equalTo(Arrays.asList(alivePatient))));
    }
    ...
    public void shouldCreateMotherChildRelationshipAndUpdate() throws ParentNotFoundException {
        ...
        when(mockEncounterService.persistEncounter(mockMRSPatient, staffId, facilityId, Constants.ENCOUNTER_PATIENTREGVISIT, new Date(), null)).thenReturn(mockMRSEncounter);
