...
public class RegisterCWCFormValidator extends FormValidator<RegisterCWCForm> {
    ...
    public List<FormError> validatePatient(Patient patient, FormWithHistoryInput formWithHistoryInput, List<FormBean> formBeans, List<FormBean> allForms) {
        ...
        final PatientValidator regClientFormValidators = new RegClientFormSubmittedInSameUpload()
                .onSuccess(new IsFormSubmittedWithValidHistoryDates(formWithHistoryInput))
            ...
            final List<FormError> errors = new DependentValidator().validate(patient, formBeans, allForms, new ExistsInDb().onSuccess(new IsAlive().onSuccess(new IsAChild().onSuccess(new HasValidHistoryDates(formWithHistoryInput)))).onFailure(regClientFormValidators));
